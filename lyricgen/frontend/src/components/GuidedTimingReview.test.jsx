...
describe("GuidedTimingReview", () => {
  it("presenta una sola decisión con lenguaje humano y audio visible", () => {
    ...
    expect(screen.getAllByText("Primera frase")).toHaveLength(2);
  ...
  it("cancela un gesto interrumpido sin guardar el movimiento", () => {
    const onMove = vi.fn();
  ...
  it("aborta el drag si una reconciliación cambió la frase durante el gesto", () => {
    const onMove = vi.fn();
  ...
  it("no reproduce una ventana que quedó fuera de la duración real", async () => {
    ...
    expect(screen.getByRole("button", { name: /reproducir este tramo en loop/i })).toBeDisabled();
  ...
  it("no permite mover el final hacia adelante sin una duración conocida", () => {
    render(<GuidedTimingReview {...props({
  ...
  it("permite arrastrar una frase sin cruzar a sus vecinas", () => {
    const onMove = vi.fn();
    render(<GuidedTimingReview {...props({
  ...
  it("respeta una frase vecina aunque quede fuera de la ventana visible", () => {
    const onMove = vi.fn();
    render(<GuidedTimingReview {...props({
