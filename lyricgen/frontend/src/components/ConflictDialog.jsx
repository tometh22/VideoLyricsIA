...
export default function ConflictDialog({ conflict, onUseServer, onSaveLocal, onCancel }) {
    ...
    <div className="fixed inset-0 z-[90] grid place-items-center bg-black/70 p-4 backdrop-blur-sm" role="presentation">
      <div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="editor-conflict-title" tabIndex={-1} className="w-full max-w-lg rounded-2xl bg-surface-1 p-6 shadow-2xl ring-1 ring-white/15 outline-none">
        ...
        <p className="mt-2 text-sm leading-relaxed text-ink-secondary">
          {actor ? `${actor} guardó cambios mientras estabas editando. ` : "Otro integrante guardó cambios. "}
        ...
        <button type="button" disabled={busy} onClick={onCancel} className="mt-3 w-full rounded-lg px-3 py-2 text-xs text-ink-tertiary hover:text-white disabled:opacity-50">Cancelar y seguir revisando</button>
      </div>
    </div>
  );
}
