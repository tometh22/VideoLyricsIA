...
export default function News() {
  const { t } = useI18n();
  const items = [
    { t: t("news.1_t"), d: t("news.1_d"), date: "May 2026", video: "/movement_samples/foto-parallax.mp4" },
    { t: t("news.2_t"), d: t("news.2_d"), date: "May 2026", video: null },
    { t: t("news.3_t"), d: t("news.3_d"), date: "Abr 2026", video: "/movement_samples/animado.mp4" },
  ];
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <article className="glass rounded-3xl overflow-hidden glass-hover lg:row-span-2 flex flex-col">
          <div className="aspect-video bg-gradient-to-br from-brand/30 to-accent/15 overflow-hidden">
            {featured.video && <video autoPlay muted loop playsInline className="w-full h-full object-cover" src={featured.video} />}
          </div>
        ...
        {rest.map((n) => (
          <article key={n.t} className="glass rounded-3xl overflow-hidden glass-hover flex gap-5 p-5">
            <div className="w-32 shrink-0 rounded-2xl overflow-hidden bg-gradient-to-br from-brand/25 to-accent/15 self-stretch flex items-center justify-center">
              {n.video ? <video autoPlay muted loop playsInline className="w-full h-full object-cover" src={n.video} /> : <span className="px-2 py-0.5 rounded-full bg-white/15 text-white text-[10px] font-bold uppercase">Nuevo</span>}
            </div>
            <div className="py-1">
