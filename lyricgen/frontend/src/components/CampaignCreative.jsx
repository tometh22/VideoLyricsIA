...
export default function CampaignCreative({ campaignId, view = "creative" }) {
  ...
  // 2026-09-14: el preset de la campaña (defaults creativos) era invisible.
  const { t: tPreset } = useI18n();
  const [presetEdit, setPresetEdit] = useState(null);      // copia editable de defaults_explicit
  const [presetCleared, setPresetCleared] = useState(new Set());
  const [presetReason, setPresetReason] = useState("");
  const presetValueLabel = (key, value) => (key === "effect" ? EFFECT_LABELS(tPreset)[value] : key === "movement_style" ? MOVEMENT_LABELS(tPreset)[value]
    : key.includes("font") ? FONT_LABELS(tPreset)[value] : AXIS_VALUE_LABELS(tPreset)[key]?.[value] || sourceLabels[value] || dynamicAxisLabel(tPreset, key, value)) || (value === "" || value == null ? "Auto / ninguno" : String(value));
  const PRESET_KEYS = ["font", "font_scale", "text_case", "style", "movement_style", "line_transition", "lyrics_animation", "effect", "background_mode", "frame_format"];
  ...
  return <section className="space-y-5" aria-label="Configuración de campaña">
    ...
    {view === "creative" && <>
      ...
      {data.can_manage && <div className="rounded-2xl bg-surface-2/30 p-4 ring-1 ring-white/10"><button type="button" aria-expanded={configurationOpen} className="flex w-full items-center justify-between gap-4 text-left" onClick={() => setConfigurationOpen(open => !open)}><span><span className="text-xs font-semibold uppercase tracking-wider text-brand-light">Paso 2 · opcional</span><strong className="mt-1 block">Configurar estilos y reparto</strong><span className="mt-1 block text-sm font-normal text-ink-secondary">Abrilo sólo si querés cambiar el diseño guardado antes de generar.</span></span><span className="text-2xl text-brand-light" aria-hidden="true">{configurationOpen ? "−" : "+"}</span></button>{configurationOpen && <fieldset disabled={busy} className="mt-5 space-y-4 border-t border-white/10 pt-5">
        <section data-testid="campaign-preset" className="space-y-3 rounded-2xl bg-brand/[0.06] p-4 ring-1 ring-brand/20">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div><h3 className="font-semibold">Preset de la campaña</h3><p className="text-sm text-ink-secondary">Se aplica a toda canción que no tenga un estilo asignado abajo. Es lo que se usa al generar si no repartís estilos.</p></div>
            {!presetEdit && <button type="button" className={button} onClick={() => { setPresetEdit({ ...(data.defaults_explicit || {}) }); setPresetCleared(new Set()); setPresetReason(""); }}>Editar preset</button>}
          </div>
          <dl className="grid gap-2 text-sm sm:grid-cols-2 lg:grid-cols-5">{PRESET_KEYS.filter(k => data.fields?.[k]).map(k => <div key={k} className="rounded-lg bg-black/20 px-3 py-2"><dt className="text-[11px] uppercase tracking-wide text-ink-tertiary">{data.fields[k].label}</dt><dd className="mt-0.5 font-medium text-white">{presetValueLabel(k, data.defaults?.[k])}{Object.hasOwn(data.defaults_explicit || {}, k) ? "" : <span className="ml-1 text-[10px] font-normal text-ink-tertiary">(por defecto)</span>}</dd></div>)}</dl>
          {presetEdit && <div className="space-y-3 border-t border-white/10 pt-3">
            <p className="text-sm text-ink-secondary">Tildá lo que querés fijar para la campaña; lo que destildás vuelve al valor por defecto del sistema.</p>
            <Controls fields={data.fields} values={presetEdit} assets={assets} label="Preset" update={(k, v) => { setPresetEdit(old => { const next = { ...old }; if (v === undefined) delete next[k]; else next[k] = v; return next; }); setPresetCleared(old => { const next = new Set(old); if (v === undefined) next.add(k); else next.delete(k); return next; }); }} />
            <label className="block">Motivo del cambio<input aria-label="Motivo del preset" className={input} value={presetReason} onChange={e => setPresetReason(e.target.value)} /></label>
            <div className="flex flex-wrap gap-3">
              <button type="button" className={primaryButton} disabled={busy || presetReason.trim().length < 3} onClick={() => run(async () => {
                const settings = { ...presetEdit };
                for (const k of presetCleared) if (!Object.hasOwn(settings, k)) settings[k] = null;
                await post(`${base}/creative/defaults`, { revision: data.plan.revision, settings, reason: presetReason });
                setPresetEdit(null); setPresetCleared(new Set()); await load(); setMessage("Preset guardado. Se aplica a las próximas generaciones; las canciones con estilo asignado no cambian.");
              })}>Guardar preset</button>
              <button type="button" className={button} disabled={busy} onClick={() => { setPresetEdit(null); setPresetCleared(new Set()); }}>Cancelar</button>
            </div>
          </div>}
        </section>
        ...
        <p className="text-sm text-ink-secondary">El reparto define <strong>qué estilo</strong> recibe cada canción seleccionada, no cuántos videos se generan. La cantidad a generar se controla arriba, en «Generar {eligible.length} videos».</p>
        <p role="status" className={`text-sm ${distributionBalanced ? "text-emerald-200" : "text-amber-200"}`}>{mode === "percent"
          ? `Suma de porcentajes: ${trim(weightSum)}% de 100%${distributionBalanced ? " · listo para repartir" : weightSum < 100 ? ` · faltan ${trim(100 - weightSum)}%` : ` · sobran ${trim(weightSum - 100)}%`}`
          : `Suma de canciones: ${trim(weightSum)} de ${selected.size} seleccionadas${distributionBalanced ? " · listo para repartir" : weightSum < selected.size ? ` · faltan ${trim(selected.size - weightSum)}` : ` · sobran ${trim(weightSum - selected.size)}`}`}</p>
        ...
        <button className={button} disabled={!selected.size || reason.trim().length < 3 || !distributionBalanced} title={distributionBalanced ? undefined : mode === "percent" ? "Los porcentajes deben sumar 100" : "Las cantidades deben sumar las canciones seleccionadas"} onClick={() => run(async () => setPreview(await post(`${base}/creative/preview`, { revision: data.plan.revision, item_ids: [...selected], mode, groups, seed, replace_exceptions: replace, pin, contract, agreement, rounding_note: rounding, reason }))) }>Ver reparto antes de guardar</button>
