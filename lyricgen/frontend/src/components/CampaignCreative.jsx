...
export default function CampaignCreative({ campaignId, view = "creative" }) {
  ...
  const approvedVideos = (report?.videos || []).filter(v => v.status === "done" && v.approved_at);
  const selectedApprovedVideos = approvedVideos.filter(v => selectedVideoIds.has(v.job_id));
  ...
  return <section className="space-y-5" aria-label="Configuración de campaña">
    ...
    {view === "history" && report && <div className="space-y-4"><h2 className="text-xl font-semibold">Videos de esta campaña ({report.videos.length})</h2><p className="text-sm text-ink-secondary">Incluye generaciones, reintentos y variantes vinculadas a esta campaña.</p>
      ...
      <div className="grid gap-4 md:grid-cols-2">{report.videos.map(v => <article key={v.job_id} className="space-y-3 rounded-xl bg-surface-2/40 p-5 ring-1 ring-white/10"><VideoThumbnail video={v} /><div><h3 className="font-semibold">{v.title}</h3><p className="text-sm text-ink-secondary">{v.artist} · {new Date(v.created_at).toLocaleString()}</p></div><p>{statusLabels[v.status] || "En preparación"}{v.parent_job_id ? " · Variante" : ""}</p><p className="text-sm">{v.assignment.group_name || "Sin clasificación contractual"}</p>{data.can_manage && v.status === "done" && v.approved_at && <label className="flex items-center gap-2 text-sm text-emerald-100"><input type="checkbox" aria-label={`Seleccionar video ${v.title}`} checked={selectedVideoIds.has(v.job_id)} onChange={e => setSelectedVideoIds(old => { const next = new Set(old); if (e.target.checked) next.add(v.job_id); else next.delete(v.job_id); return next; })} />Seleccionar para enviar</label>}<button className={button} onClick={() => navigate(v.open_path)}>Abrir video y versiones</button>
    ...
    {bulkDelivery && <div role="dialog" aria-modal="true" aria-label="Enviar videos aprobados" className="fixed inset-0 z-50 grid place-items-center bg-black/80 p-5"><div className="max-w-lg space-y-4 rounded-xl bg-surface-2 p-6"><h2>Enviar {bulkDelivery.length} videos aprobados</h2><p>Se creará una operación durable y cada video se actualizará en el portal elegido. Podés cerrar esta ventana mientras se procesa.</p><label className="block text-sm">Portal de destino<select aria-label="Portal de destino" className={input + " mt-2"} value={deliveryPortal} onChange={e => setDeliveryPortal(e.target.value)}><option value="">Elegí un portal</option>{deliveryPortals.map(portal => <option key={portal.id} value={portal.id}>{portal.label} · {portal.host}</option>)}</select></label><ul className="max-h-40 overflow-auto text-sm text-ink-secondary">{bulkDelivery.map(v => <li key={v.job_id}>{v.artist} · {v.title}</li>)}</ul><button className={primaryButton} disabled={busy || !deliveryPortal} onClick={() => run(async () => { const key = `campaign-${campaignId}-${globalThis.crypto?.randomUUID?.() || `${Date.now()}-${Math.random()}`}`; const operation = await post(`${base}/deliveries`, { job_ids: bulkDelivery.map(v => v.job_id), destination_portal: deliveryPortal, idempotency_key: key }); setBulkDelivery(null); setMessage(`Envío iniciado para ${operation.total_count || bulkDelivery.length} videos a ${deliveryPortals.find(p => p.id === deliveryPortal)?.label || deliveryPortal}. La operación sigue en segundo plano.`); })}>Confirmar envío</button><button className={button} disabled={busy} onClick={() => setBulkDelivery(null)}>Cancelar</button></div></div>}
