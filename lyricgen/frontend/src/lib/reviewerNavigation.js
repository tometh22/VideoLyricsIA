export const REVIEW_SCOPES = [["pending", "Por revisar"], ["approved", "Aprobadas"], ["all", "Todas"]];
...
export function reviewStateFilter(value, scope) {
  ...
  const allowed = scope === "all" ? [...approved, ...pending] : scope === "approved" ? approved : pending;
...
export function reviewCounts(queue, campaign) {
  ...
  return { all, approved, pending: all == null || approved == null ? undefined : Math.max(0, all - approved) };
}
