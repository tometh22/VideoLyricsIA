...
def _sample_frames(prefix, count):
    tmp_dir = tempfile.mkdtemp(prefix=prefix)
    frames = []
    for index in range(count):
        frame = os.path.join(tmp_dir, f"frame_{index}.jpg")
        open(frame, "wb").close()
        frames.append(frame)
    return frames, tmp_dir, count


def test_universal_video_requires_complete_independent_and_gemini_passes(monkeypatch):
    primary = _sample_frames("genly_primary_", 2)
    secondary = _sample_frames("genly_secondary_", 4)
    monkeypatch.setattr(content_validator, "_extract_frames", lambda _p: primary)
    monkeypatch.setattr(
        content_validator,
        "_extract_frames_with_opencv",
        lambda _p, **_kwargs: secondary,
    )
    monkeypatch.setattr(
        content_validator,
        "_check_frame_with_local_detector",
        lambda _p: {"people": False, "evidence": []},
    )
    monkeypatch.setattr(
        content_validator,
        "_check_frame_with_gemini",
        lambda _p: _classified(),
    )

    result = content_validator.validate_video(
        "fake.mp4", allow_people=False, require_secondary=True
    )

    assert result["passed"] is True
    assert result["frames_checked"] == result["frames_planned"] == 2
    assert result["secondary_frames_checked"] == 4
    assert result["secondary_frames_planned"] == 4
    assert set(result["validator_stack"]) == {
        content_validator.GEMINI_VALIDATOR_VERSION,
        content_validator.LOCAL_VALIDATOR_VERSION,
    }


def test_independent_people_detection_blocks_before_paid_gemini(monkeypatch):
    primary = _sample_frames("genly_primary_", 2)
    secondary = _sample_frames("genly_secondary_", 3)
    monkeypatch.setattr(content_validator, "_extract_frames", lambda _p: primary)
    monkeypatch.setattr(
        content_validator,
        "_extract_frames_with_opencv",
    ...
    monkeypatch.setattr(
        content_validator,
        "_check_frame_with_local_detector",
        lambda _p: {"people": True, "evidence": ["visible_face:1"]},
    )
    monkeypatch.setattr(
        content_validator,
        "_check_frame_with_gemini",
        lambda _p: (_ for _ in ()).throw(AssertionError("Gemini must not run")),
    )
    ...
    assert result["frames_checked"] == 0
    assert result["detections"]["people"] is True
    assert result["issues"][0]["validator"] == content_validator.LOCAL_VALIDATOR_VERSION


def test_independent_detector_error_fails_closed(monkeypatch):
    primary = _sample_frames("genly_primary_", 1)
    secondary = _sample_frames("genly_secondary_", 2)
    monkeypatch.setattr(content_validator, "_extract_frames", lambda _p: primary)
    monkeypatch.setattr(
        content_validator,
        "_extract_frames_with_opencv",
    ...
    monkeypatch.setattr(
        ...
        lambda _p: (_ for _ in ()).throw(
            content_validator.LocalDetectorCheckError("decoder unavailable")
        ),
    )
    ...
    assert result["passed"] is False
    assert result["secondary_errors"] == 1
    assert "fail-closed Universal policy" in result["issues"][0]["type"]


def test_universal_image_requires_both_validators(monkeypatch, tmp_path):
    image = tmp_path / "frame.jpg"
    image.write_bytes(b"jpg")
    monkeypatch.setattr(
        content_validator,
    ...
    result = content_validator.validate_image(
        str(image), allow_people=False, require_secondary=True
    )

    assert result["passed"] is True
    assert result["secondary_frames_checked"] == 1
    assert len(result["validator_stack"]) == 2


def test_yunet_threshold_rejects_historical_rain_and_moon_false_positives():
    source = __import__("inspect").getsource(
        content_validator._local_detector_assets
    )
    assert "score_threshold=0.85" in source
    assert "rain droplets" in source


def test_bundled_yunet_model_has_expected_checksum_and_loads():
    model = os.path.join(
        os.path.dirname(content_validator.__file__),
        "assets", "models", "face_detection_yunet_2023mar.onnx",
    )
    with open(model, "rb") as handle:
        digest = hashlib.sha256(handle.read()).hexdigest()

    assert digest == content_validator._YUNET_MODEL_SHA256
    cv2, detector = content_validator._local_detector_assets()
