...
def test_editor_document_is_shared_by_tenant_and_conflicts_are_explicit(client):
    ...
    token_a = create_token(first)
    token_b = create_token(second)
...
def test_editor_rejects_other_tenant_and_analytics_is_bounded(client):
    ...
    try:
        response = client.get(f"/editor/{job_id}", headers=auth(create_token(other)))
    ...
    events = client.post(
        "/analytics/events", headers=auth(create_token(first)),
        json={"events": [
