...
def test_regenerate_scene_busts_only_target(monkeypatch, tmp_path):
    ...
    assert verso_call["ns"] == (
        "A|S|verso_1|auto|background-v6:off:deny"
    )
...
def test_scene_cache_ns_token():
    base = "A|S|coro_1|auto|background-v6:off:deny"
...
def test_scene_plan_validation_requires_matching_policy_fingerprint(monkeypatch):
    ...
    fingerprint = "background-v6:enforce:deny|people:deny"
    plan = {
        "scenes": [
            {
    ...
    plan["scenes"][0]["validation"]["policy_fingerprint"] = (
        "background-v6:shadow:deny|people:deny"
    )
    assert pipeline._scene_plan_has_current_clip_validation(plan) is False
...
def test_shadow_scene_validation_is_stale_after_enforce_rollout(monkeypatch):
    ...
    plan = {
        "scenes": [{
            ...
            "validation": {
                "passed": True,
                "policy_fingerprint": "background-v6:shadow:deny|people:deny",
            },
        }]
    }
