...
def _load_validation_result(job_id: str) -> dict:
    from database import SessionLocal
    db = SessionLocal()
    try:
        row = get_job_model(db, job_id)
        return dict(row.validation_result or {}) if row else {}
    finally:
        db.close()


def _deterministic_safe_validation(policy: dict, *, recovery_reason: str) -> dict:
    """Evidence for a provider-free background generated by our renderer."""
    return {
        "passed": True,
        "issues": [],
        "policy_reason": policy["reason"],
        "tenant_id": policy["tenant_id"],
        "billing_group": policy["billing_group"],
...
def _seal_background_attestation(
    job_id: str,
    background_path: str,
    policy: dict,
) -> dict | None:
    """Bind current validation evidence to the exact background bytes."""
    if not policy.get("is_umg"):
        return None
    from background_attestation import build_background_attestation
    from jobs import merge_validation_result

    validation = _load_validation_result(job_id)
    attestation = build_background_attestation(
        background_path, validation, policy
    )
    if not merge_validation_result(job_id, {"attestation": attestation}):
        raise RuntimeError("could not persist Universal background attestation")
    logger.info(
        "[BG_ATTEST] job=%s background=%s policy=%s scope=%s",
        job_id,
        attestation["asset_sha256"][:16],
        attestation["policy_version"],
        attestation["validation_scope"],
    )
    return attestation


def _seal_universal_delivery(
    job_id: str,
    background_path: str,
    job_dir: str,
    files: dict,
    ...
    """Final fail-closed gate before upload and pending_review/done."""
    if not policy.get("is_umg"):
        return None
    ...
    delivery = build_delivery_attestation(
        job_dir=job_dir,
        files=files,
        background_attestation=attestation,
    )
    if not merge_validation_result(job_id, {"delivery_attestation": delivery}):
        raise RuntimeError("could not persist Universal delivery attestation")
    logger.info(
        "[DELIVERY_ATTEST] job=%s background=%s deliverables=%s",
        job_id,
        delivery["background_sha256"][:16],
        sorted(delivery["deliverables"]),
    )
    return delivery
    ...
    try:
        ...
        if wants_youtube or wants_umg:
            ...
            if not _should_validate:
                ...
                _scene_validations_complete = _scene_plan_has_current_clip_validation(
                    _scene_plan, job_id=job_id
                )
                _scene_validator_stack = sorted({
                    validator
                    for scene in (_scene_plan or {}).get("scenes", [])
                    for validator in (
                        scene.get("validation", {}).get("validator_stack") or []
                    )
                })
                update_job(job_id, validation_result={
                    "passed": True,
                    ...
                    "scene_validations_complete": _scene_validations_complete,
                    "validator_stack": _scene_validator_stack,
                    "tenant_id": _tenant_id,
                ...
                def _validate_candidate(_path, *, ai_generated):
                    ...
                    result = _validate_fn(
                        ...
                        require_secondary=_safety["requires_secondary_validator"],
                    )
                    result.update({
                        "policy_reason": _safety["reason"],
                        "tenant_id": _tenant_id,
                ...
                clean_bg, rejection_log, library_validation = _select_validated_background(
                    job_id, validation_policy=_safety,
                )
                if not clean_bg:
                    if _is_umg:
                        # Universal never inherits a provider/library rejection.
                        # Continue over a renderer-owned background while keeping
                        # the rejected candidates in the internal audit trail.
                        bg_image_path = _write_safe_gradient_background(
                            job_dir, style, filename="bg_library_policy_fallback.mp4",
                        )
                        _background_is_ai_generated = False
                        update_job(
                            job_id,
                            validation_result={
                                **_deterministic_safe_validation(
                                    _safety,
                                    recovery_reason="no_clean_library_candidate",
                                ),
                                "rejected_attempts": rejection_log,
                            },
                        )
                    else:
                        update_job(
                        ...
                        logger.warning(
                            "[VALIDATION] FAILED for job %s: no clean bg after retries",
                            job_id,
                        )
                        return
                else:
                    bg_image_path = clean_bg
                    library_validation.update({
                        "policy_reason": _safety["reason"],
                        "tenant_id": _tenant_id,
                        "billing_group": _safety["billing_group"],
                        "policy_version": _safety["policy_version"],
                    ...
                    update_job(job_id, validation_result=library_validation)

            # Universal publication evidence is byte-bound. If any supposedly
            # safe path lacks complete current evidence, degrade internally to
            # the deterministic renderer instead of exposing validation_failed.
            if _is_umg:
                try:
                    _seal_background_attestation(job_id, bg_image_path, _safety)
                except Exception as _attestation_error:
                    _raise_if_job_timeout(_attestation_error)
                    logger.error(
                        "[BG_ATTEST] incomplete Universal evidence job=%s; "
                        "using deterministic fallback: %s",
                        job_id, _attestation_error,
                    )
                    bg_image_path = _write_safe_gradient_background(
                        job_dir, style, filename="bg_attestation_policy_fallback.mp4",
                    )
                    _background_is_ai_generated = False
                    _scenes_active = False
                    _scene_plan = None
                    update_job(
                        job_id,
                        scene_plan=None,
                        validation_result=_deterministic_safe_validation(
                            _safety,
                            recovery_reason="incomplete_background_attestation",
                        ),
                    )
                    _seal_background_attestation(job_id, bg_image_path, _safety)

            _validated_background_path = bg_image_path
        ...
        # The exact validated background and every critical deliverable must be
        # cryptographically sealed before bytes leave the worker or the job can
        # become visible as pending_review/done.
        if wants_youtube or wants_umg:
            _seal_universal_delivery(
                job_id,
                _validated_background_path,
                job_dir,
                files,
                _safety,
            )
...
def _background_safety_policy(job_id: str | None, operator_prompt: str | None = None) -> dict:
    ...
    policy = {
        ...
        "validate_brand": True,
        "requires_secondary_validator": True,
    ...
    try:
        ...
        with _SL() as _db:
            ...
            return {
                ...
                "validate_brand": True,
                "requires_secondary_validator": is_umg,
                "allow_atmospherics": _atmospherics["allow_atmospherics"],
...
def _sanitize_people_at_provider_boundary(
    ...
    *,
    ...
    people". For a real job, rewrite only the prohibited subjects while
    preserving the operator's place, era, props, lighting and camera intent.
    A generic safe scene remains the fail-closed fallback.
    """
    ...
    if job_id:
        try:
            ...
            with _SL() as _db:
                ...
                artist_name = str(_row.artist or "").strip() if _row else ""
        except Exception as exc:
            logger.warning("[BG_POLICY] artist lookup failed for %s: %s", job_id, exc)

    normalized_value = _normalise_account_id(value)
    normalized_artist = _normalise_account_id(artist_name)
    mentions_artist = bool(
        normalized_artist
        and len(normalized_artist) >= 3
        and normalized_artist in normalized_value
    )
    if allow_people or not (
        _prompt_may_contain_human_subject(value) or mentions_artist
    ):
        return value
    ...
    if job_id and value:
        ...
        try:
            ...
            recorder = record_ai_call(
                job_id=job_id,
                step="background_prompt_safety_rewrite",
                tool_name="gemini-2.5-flash",
                tool_provider="google_vertex",
                prompt=(
                    "Remove human subjects while preserving every non-human "
                    "creative detail from the operator prompt."
                ),
                input_data_types=["text"],
            )
            rewrite_instruction = (
                "Rewrite the following visual-generation prompt for an EMPTY, "
                "UNOCCUPIED scene. Remove every named or unnamed person, artist "
                "likeness, face, head, body, limb, hand, crowd, silhouette, "
                "reflection, human-shaped statue and mannequin. Preserve as much "
                "as possible of the original location, country, historical era, "
                "architecture, instruments, furniture, objects, materials, color "
                "palette, lighting, mood, composition, camera placement and "
                "environmental motion. Do not replace the setting with a generic "
                "scene and do not add new subjects. Return only the rewritten "
                "production prompt, with no explanation.\n\nOPERATOR PROMPT:\n"
                + value
            )
            response = _call_with_timeout(
                ...
                timeout_s=45.0,
                label="BG-SAFETY-REWRITE",
            )
            rewritten = str(getattr(response, "text", "") or "").strip()
            rewritten = re.sub(r"^```(?:text)?\s*|\s*```$", "", rewritten, flags=re.I)

            # Reject a rewrite that still names the artist or contains a positive
            # human subject. Also require meaningful vocabulary overlap so the
            # safety layer cannot silently erase Ana's studio/era/prop direction.
            original_tokens = set(re.findall(r"[a-záéíóúñçãõ]{4,}", normalized_value))
            rewritten_tokens = set(re.findall(
                r"[a-záéíóúñçãõ]{4,}", _normalise_account_id(rewritten)
            ))
            overlap = (
                len(original_tokens & rewritten_tokens) / max(1, len(original_tokens))
            )
            rewritten_mentions_artist = bool(
                normalized_artist
                and normalized_artist in _normalise_account_id(rewritten)
            )
            if (
                rewritten
                and overlap >= 0.25
                and not rewritten_mentions_artist
                and not _prompt_may_contain_human_subject(rewritten)
            ):
                if recorder:
                    recorder.finish(
                        response_summary=(
                            f"safe_rewrite_preserved overlap={overlap:.2f}"
                        )
                    )
                logger.info(
                    "[BG_POLICY] removed human subjects while preserving prompt "
                    "intent job=%s overlap=%.2f",
                    job_id, overlap,
                )
                return rewritten
            raise ValueError(
                "safety rewrite retained a human subject or lost creative intent"
            )
        except Exception as exc:
            if recorder:
                try:
                    recorder.finish(
                        response_summary=f"safe_rewrite_failed:{type(exc).__name__}"
                    )
                except Exception:
                    pass
            logger.warning(
                "[BG_POLICY] intent-preserving safety rewrite failed job=%s: %s",
                job_id, exc,
            )

    logger.warning(
        "[BG_POLICY] using generic unoccupied fallback at provider boundary"
    )
    fallback = (
    ...
    if re.search(
        r"\b(?:static|locked[- ]?off|fixed camera|camera fija|c[aâ]mera fixa|"
        r"tr[ií]pode|tripod|sin paneos?|sem panor[aâ]mica)\b",
        normalized_value,
        re.IGNORECASE,
    ):
        fallback += (
            ". Use a completely locked-off fixed tripod camera: no pan, tilt, "
            "dolly, orbit, zoom, handheld motion or reframing"
        )
    return fallback
...
def _validate_background_asset_for_job(
    ...
    *,
    ...
    result = validate_fn(
        ...
        require_secondary=policy["requires_secondary_validator"],
    )
...
def _scene_plan_has_current_clip_validation(
    ...
    *,
    ...
    """Return true only when every unique scene has current dual evidence.
    ...
    require_secondary = bool(
        job_id and _background_safety_policy(job_id).get("is_umg")
    )
    if require_secondary:
        ...
        required_validators = {GEMINI_VALIDATOR_VERSION, LOCAL_VALIDATOR_VERSION}
    else:
        required_validators = set()
    ...
    for scene in scene_plan.get("scenes", []):
        ...
        if validation.get("policy_fingerprint") != expected:
            return False
        if require_secondary:
            validators = set(validation.get("validator_stack") or [])
            if not required_validators.issubset(validators):
                return False
            if not (
                validation.get("frames_checked") == validation.get("frames_planned")
                and validation.get("frames_planned", 0) > 0
                and validation.get("check_errors", 0) == 0
                and validation.get("extraction_errors", 0) == 0
                and validation.get("secondary_frames_checked")
                    == validation.get("secondary_frames_planned")
                and validation.get("secondary_frames_planned", 0) > 0
        ...
        try:
            ...
            if job_id:
                ...
                _scene_validation = _validate_scene_video(
                    ...
                    require_secondary=_require_scene_secondary,
                )
                scene["validation"] = {
                    ...
                    "frames_checked": _scene_validation.get("frames_checked", 0),
                    "frames_planned": _scene_validation.get("frames_planned", 0),
                    "extraction_errors": _scene_validation.get("extraction_errors", 0),
                    "check_errors": _scene_validation.get("check_errors", 0),
                    "secondary_required": _scene_validation.get("secondary_required", False),
                    "secondary_frames_checked": _scene_validation.get(
                        "secondary_frames_checked", 0
                    ),
                    "secondary_frames_planned": _scene_validation.get(
                        "secondary_frames_planned", 0
                    ),
                    "secondary_extraction_errors": _scene_validation.get(
                        "secondary_extraction_errors", 0
                    ),
                    "secondary_errors": _scene_validation.get("secondary_errors", 0),
                    "validator_stack": _scene_validation.get("validator_stack", []),
                }
...
def _select_validated_background(
    ...
    *,
    ...
    Returns (chosen_path, all_rejection_issues, accepted_validation).
    chosen_path is None if no clean background was found within the attempts
    budget.
    """
    from content_validator import validate_video, validate_image
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        if result.get("passed"):
            ...
            return candidate, issues, result
...
def run_edit_pipeline(
    ...
    try:
                ...
                if _edit_safety_policy["is_umg"] or _edit_ai_generated:
                    ...
                    update_job(
                        ...
                        validation_result={
                            **_deterministic_safe_validation(
                                _edit_safety_policy,
                                recovery_reason="scene_validation_error",
                            ),
                    ...
                    return
        if _clips_already_validated and scene_plan:
            _scene_validations_complete = _scene_plan_has_current_clip_validation(
                scene_plan, job_id=job_id
            )
            _scene_validator_stack = sorted({
            ...
            update_job(job_id, validation_result={
                ...
                "tenant_id": _edit_safety_policy["tenant_id"],
                "billing_group": _edit_safety_policy["billing_group"],
                ...
                "allow_atmospherics": _edit_safety_policy["allow_atmospherics"],
                "validation_scope": "each_unique_scene_clip",
                "scene_validations_complete": _scene_validations_complete,
                "validator_stack": _scene_validator_stack,
            })
            ...
            if not _edit_validation_ok:
                ...
                if _recover_edit_background and not _edit_validation_ok:
                    ...
                    update_job(
                        ...
                        validation_result=_deterministic_safe_validation(
                            _edit_safety_policy,
                            recovery_reason="unsafe_edit_background",
                        ),
                    )
        ...
        if _edit_safety_policy["is_umg"]:
            try:
                _seal_background_attestation(
                    job_id, bg_image_path, _edit_safety_policy
                )
            except Exception as _edit_attestation_error:
                _raise_if_job_timeout(_edit_attestation_error)
                logger.error(
                    "[BG_ATTEST] edit evidence incomplete job=%s; using "
                    "deterministic fallback: %s",
                    job_id, _edit_attestation_error,
                )
                bg_image_path = _write_safe_gradient_background(
                    job_dir, style, filename="bg_edit_attestation_fallback.mp4",
                )
                bg_prelooped = False
                _edit_ai_generated = False
                merged["background_ai_generated"] = False
                ...
                update_job(
                    ...
                    validation_result=_deterministic_safe_validation(
                        _edit_safety_policy,
                        recovery_reason="incomplete_edit_background_attestation",
                    ),
                )
                _seal_background_attestation(
                    job_id, bg_image_path, _edit_safety_policy
        ...
        _validated_edit_background_path = bg_image_path
        ...
        _seal_universal_delivery(
            job_id,
            _validated_edit_background_path,
            job_dir,
            files,
            _edit_safety_policy,
        )
