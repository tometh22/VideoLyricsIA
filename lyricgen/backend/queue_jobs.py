...
def enqueue_bg_preview(
    ...
    if q_default is not None:
        ...
        # con JobTimeoutException antes de que Veo termine. 900s deja margen
        # para hasta 2 generaciones (1 do-over: retry transitorio o re-roll
        # por calidad) + upload a R2. El loop interno de Veo ahora hace 1 retry
        # (2 intentos, antes 3). Nota: las fallas de Veo caen al gradient
        # fallback (no re-lanzan), así que el RQ Retry(max=2) de abajo NO
        # reintenta Veo — sólo cubre fallos de R2/infra que sí propagan.
        # (Sentry "Veo 3 JobTimeoutException 300s")
