...
Tenant scoping (UMG):
  By default, generated assets are tagged with `owner_tenant_id` from the
  LIBRARY_OWNER_TENANT_ID env var (set this to the UMG tenant_id when
  seeding their library). Pass an empty string to generate global assets
  visible to every tenant. Override per-run with --tenant-id <name>.
...
# Job before it may reach Vertex. JSON/SQL checkpoints still protect completed
# assets from later metadata-write failures.
try:
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="owner_tenant_id to assign every generated asset (default: env "
             ...
             "string to generate global (visible-to-all) assets.",
    )
        ...
        for idx, (concept, asset_type, prompt) in enumerate(SEED, start=1):
            ...
            try:
                tracking_job_id = ensure_internal_tracking_job(
                    f"library-core:{concept}:{asset_type}", db=db,
                    style=asset_type)
                if asset_type == "image":
