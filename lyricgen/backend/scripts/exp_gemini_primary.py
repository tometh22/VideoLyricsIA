...
def transcribe(audio_path: str, artist: str, song: str, *, model: str,
    ...
    # onset snap (R&D helper no presente en staging — el CTC re-timea después)
    if vocal_regions:
        try:
            import forced_align
            segs = forced_align.snap_starts_to_vocal_onsets(
...
def main():
    ...
    dt = time.time() - t0
    # save-first: el scoring legacy depende de firmas de score_benchmark que
    # cambiaron — su crash no puede costarnos la transcripción (cara).
    if args.save:
        Path(args.save).write_text(json.dumps(
            {"source": f"gemini_primary:{args.model}", "segments": segs},
            ensure_ascii=False, indent=1))
        print(f"transcript guardado: {args.save} ({len(segs)} líneas, {dt:.0f}s)")
    try:
        m = score(args.slug, segs)
    except Exception as e:
        print(f"scoring legacy falló ({e}) — transcript ya guardado, seguimos")
        return
