...
(async () => {
  ...
  // 16:9 ratio but tighter — less empty space
  ...
  // Optimize for PDF: hide nav, swap form for contact info, reduce padding
  await page.evaluate(() => {
    const nav = document.getElementById('nav');
    if (nav) nav.style.display = 'none';
    // Show contact info, hide form (form doesn't work in PDF)
  ...
  for (let i = 0; i < slideCount; i++) {
    const ssPath = path.join(__dirname, `_slide_${i + 1}.png`);
    ...
    // Screenshot the viewport (exactly what you see)
  ...
  // Create a new HTML with just the images, one per page
  const imagesHtml = screenshotPaths.map((p, i) => `
  ...
  const pdfHtmlPath = path.join(__dirname, '_pdf_temp.html');
  ...
  // Open the images HTML and export as PDF
  ...
  await pdfPage.pdf({
    path: path.join(__dirname, 'GenLy_AI_Pitch.pdf'),
    width: '1280px',
    height: '720px',
  ...
  console.log(`\nPDF generated with ${slideCount} pages: GenLy_AI_Pitch.pdf`);
  await browser.close();
})();
